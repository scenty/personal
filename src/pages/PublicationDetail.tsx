      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Button asChild variant="ghost" className="mb-6">
          <Link to="/publications">
            <ArrowLeft className="mr-2 w-4 h-4" /> 返回论文列表
          </Link>
        </Button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <Card>
              <CardContent className="p-8">
                ...
                <div className="mb-6">
                  <p className="text-lg text-muted-foreground">{publication.authors}</p>
                </div>
                ...
                {publication.abstract && (
                  <div className="mb-8">
                    <h2 className="text-xl font-semibold mb-3">摘要</h2>
                ...
                {publication.keywords && (
                  <div className="mb-6">
                    <h2 className="text-lg font-semibold mb-3">关键词</h2>
                ...
                {publication.highlights && (
                  <div className="mb-6">
                    <h2 className="text-lg font-semibold mb-3">亮点</h2>
                ...
                {publication.newsCoverage && (
                  <div className="mb-6">
                    <h2 className="text-lg font-semibold mb-3 flex items-center gap-2">
                      <Newspaper className="w-5 h-5" /> 媒体报道
                    </h2>
                    <ul className="space-y-2">
          ...
          <div className="space-y-6">
                ...
                {publication.doiLink && (
                  <Button asChild className="w-full" variant="outline">
                    <a href={publication.doiLink} target="_blank" rel="noopener noreferrer">
                      <ExternalLink className="mr-2 w-4 h-4" /> 查看论文 (DOI)
                    </a>
                  </Button>
                )}
                {publication.pdfLink && (
                  <Button asChild className="w-full" variant="outline">
                    <a href={publication.pdfLink} target="_blank" rel="noopener noreferrer">
                      <FileText className="mr-2 w-4 h-4" /> PDF下载
                    </a>
                  </Button>
                )}
                {publication.dataLink && (
                  <Button asChild className="w-full" variant="outline">
                    <a href={publication.dataLink} target="_blank" rel="noopener noreferrer">
                      <Database className="mr-2 w-4 h-4" /> 数据下载
                    </a>
                  </Button>
                )}
                {publication.codeLink && (
                  <Button asChild className="w-full" variant="outline">
                    <a href={publication.codeLink} target="_blank" rel="noopener noreferrer">
                      <Code className="mr-2 w-4 h-4" /> 代码仓库
                    </a>
                  </Button>
                )}
              </CardContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle>引用信息</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="bg-muted p-4 rounded-lg text-sm font-mono break-all">
                  {publication.authors} ({publication.year}). {publication.title}. <em>{publication.journal}</em>
